...
- **Финиш задачи**: после коммитов и проверок агент переносит `tasks/inProgress/` → `tasks/completed/`
- В конце агент **дописывает отчёт в файл задачи** (см. шаблон ниже).
...
> Перемести задачу в `tasks/completed/` и допиши отчёт: что сделано/как проверить/коммиты/риски.
