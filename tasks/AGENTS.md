...
- PR создан и прошёл ревью/слияние;
- пользователь вручную подтвердил перенос.
...
- В начале файла таски добавь блок:
  ...
  - `Owner: <agent>`
  - `Started: YYYY-MM-DD`
  - `Branch: task/<TASK_ID>-<short-slug>`
...
4) **Close**
- Создай PR (1 PR = 1 таска), запушь ветку.
- Добавь в таску отчёт и отметь `Status: ready for review`.
- Задачу в `completed/` переносит пользователь после ручной проверки/слияния.
...
### PR описание
- На русском языке: заголовок и описание.
...
- [ ] Я описал как проверить руками и создал PR.
- [ ] Я оставил отчёт в задаче и выставил `Status: ready for review`.
